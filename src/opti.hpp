...
using UpdateFunc = function<void(Vec)>;
...
public :
    ...
    static Vec quasi_newton(const Func f, const Vec& x0, const Mat& H0_inv, 
                            ...
                            int max_iter = 200, float tol = 1e-3f);
    static Vec L_BFGS(const Func f, const Vec& x0, const Mat& H0_inv, int m, int max_iter = 200, float tol = 1e-3f);
    static Vec L_BFGS_B(const Func f, const Vec& x0, const Mat& H0_inv, int m, const Vec& l, const Vec& u,
                        int max_iter = 200, float tol = 1e-3f, UpdateFunc update = nullptr);
};
